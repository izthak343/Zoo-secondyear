...
public class AddAnimalDialog extends JDialog implements ItemListener, ActionListener {
	...
	private String[] animals = {"Elephant","Giraffe","Turtle"};
	private String chosenAnimal;
	private String c;
	private JRadioButton[] rb;
	private JRadioButton[] rb1;
	private JRadioButton animalBt;
	private ZooPanel ap;
	private Animal an;
	private boolean flag=false;
	private String factoryName;
	private AbstractZooFactory fac;

	public AddAnimalDialog(ZooPanel pan, String title,AbstractZooFactory factory,String facName)
	{
		...
		p4.setLayout(new GridLayout(3,2,5,5));// choose animals
		...
		if(facName == "HerbivoreFactory") // choose animal by factory type
		{
		...
		else if(facName=="OmnivoreFactory")
		{
			radioButton("Bear");
		}
		else
	...
	public void radioButton(String anName)
	{
		animalBt=new JRadioButton(anName,false);
		animalBt.addItemListener(this);
		animalBt.setSelected(true);
		rbg1.add(animalBt);
		p4.add(animalBt);
		chosenAnimal=anName;
	}
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		{
			switch(factoryName){
			case "HerbivoreFactory":
				an=fac.produceAnimal(chosenAnimal);
				break;
			case "OmnivoreFactory":
				an=fac.produceAnimal(chosenAnimal);
				break;
			case "CarnivoreFactory":
				an=fac.produceAnimal(chosenAnimal);
				break;
			}

			ap.addAnimal(an,sl_sz.getValue(),sl_hor.getValue(),sl_ver.getValue(),c);
			setVisible(false);
		}
		else 
			setVisible(false);
